...
from util.dataset_generator_MM_custom import MatchMismatchDataGenerator, default_batch_equalizer_fn_CCE, create_tf_dataset
...
if __name__ == "__main__":
    ...
    for window_s in window_s_:
        number_mismatch = 1
        window_length = window_s * 64  # 3 seconds
        ...
        batch_size = 64
        only_evaluate = False
        training_log_filename = "training_log_{}_{}.csv".format(number_mismatch, window_s)
        results_filename = 'eval_{}_{}_s.json'.format(number_mismatch, window_s)
        ...
        results_folder = os.path.join(experiments_folder, "results_dilated_convolutional_model_{}_MM_{}_s".format(number_mismatch, window_s))
        ...
        model_path = os.path.join(results_folder, "model_{}_MM_{}_s.h5".format(number_mismatch, window_s))
        ...
        if only_evaluate:
            ...
            dataset_train = create_tf_dataset(train_generator, window_length, default_batch_equalizer_fn_CCE, hop_length, batch_size, number_mismatch=number_mismatch)

            # Create the generator for the validation set
            val_files = [x for x in glob.glob(os.path.join(data_folder, "val_-_*")) if os.path.basename(x).split("_-_")[-1].split(".")[0] in features]
            val_generator = MatchMismatchDataGenerator(val_files, window_length, spacing=spacing)
            dataset_val = create_tf_dataset(val_generator, window_length, default_batch_equalizer_fn_CCE, hop_length, batch_size, number_mismatch=number_mismatch)
        ...
        for sub in subjects:
            ...
            datasets_test[sub] = create_tf_dataset(test_generator, window_length, default_batch_equalizer_fn_CCE, hop_length, batch_size=1, number_mismatch= number_mismatch)
