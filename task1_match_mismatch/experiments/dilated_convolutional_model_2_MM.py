...
from task1_match_mismatch.models.dilated_convolutional_model_2_MM import dilation_model_2_MM
from util.dataset_generator_MM_custom import MatchMismatchDataGenerator, two_mm_CCE_label_batch_equalizer_fn, create_tf_dataset
...
if __name__ == "__main__":
    ...
    for window_length_s in window_length_s_:
        ...
        for number_mismatch in number_mismatchs:
            ...
            model = dilation_model_2_MM(time_window=window_length, eeg_input_dimension=64, env_input_dimension=stimulus_dimension)
            ...
            if only_evaluate:
                ...
                import pdb
                dataset_train = create_tf_dataset(train_generator, window_length, two_mm_CCE_label_batch_equalizer_fn, hop_length, batch_size, number_mismatch=number_mismatch)

                # Create the generator for the validation set
                val_files = [x for x in glob.glob(os.path.join(data_folder, "val_-_*")) if os.path.basename(x).split("_-_")[-1].split(".")[0] in features]
                val_generator = MatchMismatchDataGenerator(val_files, window_length, spacing=spacing)
                dataset_val = create_tf_dataset(val_generator, window_length, two_mm_CCE_label_batch_equalizer_fn, hop_length, batch_size, number_mismatch=number_mismatch)
                ...
                for sub in subjects:
                    ...
                    datasets_test[sub] = create_tf_dataset(test_generator, window_length, two_mm_CCE_label_batch_equalizer_fn,
