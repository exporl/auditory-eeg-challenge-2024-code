...
def get_hop_length(arg, data_dict):
    return int((1 / 1200) * data_dict["stimulus_sr"])
...
def run_preprocessing_pipeline(
    ...
    stimulus_steps = DefaultPipeline(
        steps=[
            ...
            ResamplePoly(1024, data_key = ['spectrogram_data', 'stimulus_data'], sampling_frequency_key = ['spectrogram_sr', 'stimulus_sr'], axis=0),
            DefaultSave(
                preprocessed_stimuli_dir,
...
if __name__ == "__main__":
    ...
    preprocessed_stimuli_folder = os.path.join(
        derivatives_folder, config["preprocessed_stimuli_folder_1024"]
    )
    preprocessed_eeg_folder = os.path.join(
        derivatives_folder, config["preprocessed_eeg_folder_1024"]
    )
