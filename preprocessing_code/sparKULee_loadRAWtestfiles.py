...
This script runs the necessary prereprocessing steps on the sparrKULee dataset, starting from the raw caches,
to arrive at the fully preprocessed files.
The raw caches are downloaded from the challenge website and should be placed in the folder specified by the
...
On the raw caches, the following preprocessing steps have been performed:
(ie. synched with the stimulus data
and loaded into python)

You are free to use these caches for your own preprocessing pipeline,
however, if you want to use certain artifact steps, such as the artifact removal MWF,
be aware that the output from these steps might differ when using input windows of just 5 seconds.
If you want to use our artificat removal steps, we recommend to use the MWF caches,
on which the artifact removal MWF has already been performed, and not compute it yourself.
The MWF caches are saved at 1024Hz, see sparKULee_mwf.py for more information on how to use these caches.
...
def temp_unpack_data(data_path):
    ...
    # save all keys, values in separate data path. TODO: fix this such that we don't have to do this
    for key, value in data.items():
        np.save(os.path.dirname(data_path) + key + '.npy', value)
...
if __name__ == "__main__":
    ...
    raw_eeg_dir = os.path.join(test_folder, task, 'raw_eeg')
    # unpack the data

    raw_eeg_data = glob.glob(os.path.join(raw_eeg_dir,  '*_eeg_raw.npz'))
