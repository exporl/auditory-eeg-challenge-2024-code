...
@tf.function
def four_mm_CCE_batch_equalizer_fn(*args):
    """Batch equalizer.
    ...
    """
    eeg = args[0]
    new_eeg = tf.concat([eeg, eeg, eeg, eeg, eeg], axis=0)
    all_features = [new_eeg]


    ### HERE ADDED 6 COMBINATIONS CORRESPONDING TO ALL POSSIBLE ORDERINGS WITH 2 MISMATCHED SEGMENTS
    for match, mismatch1, mismatch2, mismatch3, mismatch4 in zip(args[1::5], args[2::5], args[3::5], args[4::5], args[5::5]):
        stimulus_feature1 = tf.concat([match, mismatch1, mismatch2, mismatch3, mismatch4], axis=0)
        stimulus_feature2 = tf.concat([mismatch4, match, mismatch1, mismatch2, mismatch3], axis=0)
        stimulus_feature3 = tf.concat([mismatch3, mismatch4, match, mismatch1, mismatch2], axis=0)
        stimulus_feature4 = tf.concat([mismatch2, mismatch3, mismatch4, match, mismatch1], axis=0)
        stimulus_feature5 = tf.concat([mismatch1, mismatch2, mismatch3, mismatch4, match], axis=0)

        all_features += [stimulus_feature1, stimulus_feature2, stimulus_feature3, stimulus_feature4, stimulus_feature5]

    ### LABELS ? See how tile function works, dimensions of the label should be equal to the first axis of each stimulus_featureX
    labels = tf.concat(
        [
            tf.tile(tf.constant([[1, 0, 0, 0, 0]]), [tf.shape(eeg)[0], 1]), #[1 0 0 ]
            tf.tile(tf.constant([[0, 1, 0, 0, 0]]), [tf.shape(eeg)[0], 1]), #[ 0 1 0]
            tf.tile(tf.constant([[0, 0, 1, 0, 0]]), [tf.shape(eeg)[0], 1]),
            tf.tile(tf.constant([[0, 0, 0, 1, 0]]), [tf.shape(eeg)[0], 1]),#[ 0 0 1]
            tf.tile(tf.constant([[0, 0, 0, 0, 1]]), [tf.shape(eeg)[0], 1])  # [ 0 0 1]
        ],
        axis=0,
    )
...
def two_mm_CCE_label_batch_equalizer_fn(*args):
    """Batch equalizer.
    Prepares the inputs for a model to be trained in
    match-mismatch task. It makes sure that match_env
    and mismatch_env are equally presented as a first
    ...
    eeg = args[0]
    new_eeg = tf.concat([eeg, eeg, eeg], axis=0)
    all_features = [new_eeg]


    ### HERE ADDED 6 COMBINATIONS CORRESPONDING TO ALL POSSIBLE ORDERINGS WITH 2 MISMATCHED SEGMENTS
    for match, mismatch1, mismatch2 in zip(args[1::3], args[2::3], args[3::3]):
        stimulus_feature1 = tf.concat([match, mismatch1, mismatch2], axis=0)
        stimulus_feature2 = tf.concat([mismatch2, match, mismatch1], axis=0)
        stimulus_feature3 = tf.concat([mismatch1, mismatch2, match], axis=0)
       # stimulus_feature4 = tf.concat([mismatch1, match, mismatch2], axis=0)
        #stimulus_feature5 = tf.concat([mismatch2, match, mismatch1], axis=0)
        #stimulus_feature6 = tf.concat([mismatch2, mismatch1, match], axis=0)
        all_features += [stimulus_feature1, stimulus_feature2, stimulus_feature3]

    ### LABELS ? See how tile function works, dimensions of the label should be equal to the first axis of each stimulus_featureX
    labels = tf.concat(
        [
            tf.tile(tf.constant([[1, 0, 0]]), [tf.shape(eeg)[0], 1]), #[1 0 0 ]
            tf.tile(tf.constant([[0, 1, 0]]), [tf.shape(eeg)[0], 1]), #[ 0 1 0]
            tf.tile(tf.constant([[0, 0, 1]]), [tf.shape(eeg)[0], 1]) #[ 0 0 1]
        ],
        axis=0,
    )
...
@tf.function
def default_batch_equalizer_fn_CCE(*args):
    """Batch equalizer.
    Prepares the inputs for a model to be trained in
    match-mismatch task. It makes sure that match_env
    and mismatch_env are equally presented as a first
    ...
    labels = tf.concat(
        [
            tf.tile(tf.constant([[1, 0]]), [tf.shape(eeg)[0], 1]),
            tf.tile(tf.constant([[0, 1]]), [tf.shape(eeg)[0], 1]),
        ],
        axis=0,
    )
