...
path_labels_match_mismatch = os.path.join(os.path.dirname(__file__), 'labels_match_mismatch')
path_save_scores = os.path.join(os.path.dirname(__file__), 'results_task_match_mismatch')
        ...
        with open(file, 'r') as f:
            ...
            for key, label in labels.items():
                ...
                if key in predicted_labels:
                    score += (int(predicted_labels[key]) == label)
                else:
                    score += 0

            # print(f'{subject}: {score/len(labels)}')
            score_per_subject[subject] = score/len(labels)

    # print the average score for subjects in the subjects_training
    scores_training = [100*score_per_subject[x] for x in subjects_training]
    scores_test = [100*score_per_subject[x] for x in subjects_heldout]
...
for group_results in glob.glob(os.path.join(predicted_results_folder, '*')):
    if os.path.isdir(group_results):
        ...
        group_name = group_results.split('/')[-1].split('group_')[-1].split('_submission')[0]
...
with open(os.path.join(path_save_scores, 'scores_task1.json'), 'w') as f:
...
with open(os.path.join(path_save_scores,
        'scores_task1_means.json'), 'w') as f:
